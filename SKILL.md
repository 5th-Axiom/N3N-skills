...
WebSocket 维护两条独立连接，需要先启动后台服务，Agent 通过定时任务轮询获取新消息。
...
3. 比较两者：相同则跳过，不同则重新下载二进制文件并更新 `client_md5`
4. 如有更新，重启 WebSocket 服务

> 定时任务必须写入持久化配置（心跳文件、AGENTS.md、crontab 等），不能仅在当前会话中创建。
...
7. **持久化定时任务** - 定时任务写入文件或系统调度器，不依赖会话内存
8. **找人默认走任务系统** - 用户提到"找人"时使用 `task create`，仅在用户明确要在广场找人时使用 `square search`
